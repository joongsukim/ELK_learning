...
- aggregations만 사용할때 "size: 0"을 지정하면 hits에는 검색 결과 데이터를 제공하는데 그것을 제공하지 않음으로써 간편하게 결과 값을 확인가능   

- put vs post  
  put은 자원의 식별자를 알고 있을때 사용하고 post는 자원의 식별자를 모를때 사용한다.둘다 새로운 자원을 생성하는 것은 같지만 예를 들어 두개의 동일한 요청을 할 경우 post는 식별자가 없으므로 두개의 새로
  운 자원을 생성한다. 하지만 put은 식별자를 가지고 요청메세지를 보내므로 동일한 요청이 두번 들어오면 새로운 자원을 두개 만드는게 아니라 하나를 만들고 
